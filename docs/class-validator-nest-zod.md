...
1. Nest가 기본으로 class-validator를 추천하는 이유는 뭘까? (역사적·생태계·데코레이터 일관성?)
2. “타입과 검증이 한곳에서 나온다”는 게 왜 버그를 줄이는 데 도움이 될까?
3. Zod의 `refine` / `superRefine`과 class-validator의 커스텀 데코레이터는 어떤 문제를 각각 잘 풀까?
4. Swagger 문서를 “스키마에서 자동 생성”할 때, class-validator DTO와 nest-zod 스키마 중 어떤 게 유지보수하기 쉬울 수 있을까?
5. 응답 바디까지 스키마로 검증(ZodSerializerInterceptor)하는 게 필요한 경우와 오버헤드가 되는 경우는 언제일까?
